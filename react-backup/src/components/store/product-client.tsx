...
export function ProductClient({
  ...
  // Sync active image with variant image selection
  useEffect(() => {
  ...
  const discountPercent = useMemo(() => {
    ...
    if (!product.compareAtPrice || product.compareAtPrice <= activePrice) return 0;
    return Math.round(((product.compareAtPrice - activePrice) / product.compareAtPrice) * 100);
  ...
  // GA4 Ecommerce view_item event push
  useEffect(() => {
    if (product) {
        ...
        items: [
          {
            ...
            item_variant: selectedVariant ? selectedVariant.name : "Standard",
            price: product.price,
            quantity: 1,
          },
        ],
  ...
  }, [product.id, product.name, variantId]);

  const activeImageUrl = product.images[activeImgIndex]?.url || selectedVariant?.image || product.images[0]?.url || "https://images.unsplash.com/photo-1553062407-98eeb64c6a62?q=80&w=600&auto=format&fit=crop";
    ...
    <div className="bg-[#F8FAFC] min-h-screen text-slate-800 pb-16">
      ...
      <div className="bg-white border-b border-slate-100 py-3">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 text-xs font-bold text-slate-400 flex items-center gap-1.5 flex-wrap">
          ...
          <Link to={`/category/${product.categorySlug}`} className="hover:text-primary transition-colors">{product.categoryName}</Link>
          <ChevronRight className="w-3.5 h-3.5" />
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-1.5 text-xs font-bold text-slate-400">
                <span>Category:</span>
                <Link to={`/category/${product.categorySlug}`} className="text-primary hover:underline lowercase font-extrabold">
            ...
            <div className="space-y-3.5 pt-4 border-t border-slate-100">
              ...
              <div className="flex gap-3.5 w-full items-center">
                ...
                <div className="flex items-center border border-slate-200/80 rounded-full h-12 bg-slate-50/50 w-[110px] sm:w-28 shrink-0 justify-between px-1 shadow-sm">
                  <button
                    ...
                    className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-slate-200/50 font-bold text-base transition-colors cursor-pointer text-slate-500 active:scale-90 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    -
                  </button>
                ...
                <button
                  ...
                  disabled={selectedVariant && !selectedVariant.inStock}
                  className="flex-1 h-12 bg-[#1E293B] hover:bg-slate-800 text-white font-extrabold text-xs uppercase tracking-wider rounded-full flex items-center justify-center gap-2 transition-all active:scale-[0.98] cursor-pointer shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <ShoppingCart className="w-4 h-4" />
                  {selectedVariant && !selectedVariant.inStock ? "STOCK OUT" : "ADD TO CART"}
                </button>
                ...
                <button
                  ...
                  disabled={selectedVariant && !selectedVariant.inStock}
                  className="hidden sm:flex flex-1 h-12 bg-primary text-primary-foreground hover:opacity-95 font-black text-xs uppercase tracking-wider rounded-full items-center justify-center gap-2 transition-all active:scale-[0.98] cursor-pointer shadow-md shadow-primary/15 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <Zap className="w-4 h-4 fill-current" />
                  {selectedVariant && !selectedVariant.inStock ? "STOCK OUT" : "BUY NOW"}
                </button>
              </div>
              ...
              <button
                onClick={handleBuyNow}
                disabled={selectedVariant && !selectedVariant.inStock}
                className="flex sm:hidden w-full h-12 bg-primary text-primary-foreground hover:opacity-95 font-black text-xs uppercase tracking-wider rounded-full items-center justify-center gap-2 transition-all active:scale-[0.98] cursor-pointer shadow-md shadow-primary/15 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <Zap className="w-4 h-4 fill-current" />
                {selectedVariant && !selectedVariant.inStock ? "STOCK OUT" : "BUY NOW"}
              </button>
          ...
          <div className="space-y-4">
            ...
            <div className="border-2 border-dashed border-slate-200/80 bg-slate-50/50 p-5 rounded-2xl space-y-4 text-xs font-semibold text-slate-700 leading-relaxed shadow-[inset_0_2px_4px_rgba(0,0,0,0.01)]">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs pl-6 text-slate-700 font-medium leading-normal">অর্ডার করতে অগ্রিম ১ টাকাও দিতে হবেনা, পণ্য রিসিভ করার সময় টাকা পরিশোধ করবেন!</p>
              </div>

              <div className="space-y-1">
                <p className="text-slate-900 font-bold text-sm flex items-center gap-1.5">
                  🔎  প্রোডাক্ট চেক করে নেওয়ার সুযোগ:
                </p>
      ...
      {displayRelated.length > 0 && (
        <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between border-b border-slate-200 pb-3.5 mb-8">
            ...
            <Link to="/shop" className="text-xs font-bold text-slate-500 hover:text-slate-900 transition-colors uppercase tracking-wider">
              View All
            </Link>
          </div>
